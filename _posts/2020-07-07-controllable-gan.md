...
![vr]({{ site.url }}/assets/posts/2020-07-07-controllable-gan/vr.gif){: .align-center}
...
![gan]({{ site.url }}/assets/posts/2020-07-07-controllable-gan/vanilla_gan.gif){: .align-center}
...
*Can we combine the advantages of classical rendering pipelines and GANs and learn 3D controllable image synthesis from raw 2D image observations?*
...
This is a very challenging task. In this work, we made a first step towards this goal and demonstrated that it is indeed possible to learn a 3D controllable generative model from only 2D images on simple datasets, including both synthetic and real-world datasets.

![results]({{ site.url }}/assets/posts/2020-07-07-controllable-gan/results.gif){: .align-center}
...
Next, we project the 3D representations to the 2D image space via __a differentiable renderer__. 
Finally, these projected 3D representations are converted into a 2D image using a __2D Generator__. 
This is in contrast to vanilla GANs that directly generator images using 2D generators. 

![overview]({{ site.url }}/assets/posts/2020-07-07-controllable-gan/pipeline_comparison_p3.svg){: .align-center}
...
Existing approaches incorporating 3D representations into GANs consider the 3D scene as a whole.
This does not allow us to control each object individually. We propose to generate a set of __primitives__ $\mathcal{O}=\\\{\mathbf{o}_1, \dots, \mathbf{o}_N, \mathbf{o}\_{bg} \\\}$ using the 3D generator.
...
<img src="{{ site.url }}/assets/posts/2020-07-07-controllable-gan/network_3d.svg" width="50%" />
</p>
...
<p style="text-align: center">
<img src="{{ site.url }}/assets/posts/2020-07-07-controllable-gan/network_2d.svg" width="100%" />
</p>

### Why do we need the 2D Generator?
You might also wonder why do we even need the 2D Generator. 
Can we simply learn the object shape and appearance in 3D directly? 
In this case, the projected 3D primitives are directly composed as a 2D image that is fed into the discriminator. 
...
We consider two additional regularization terms in addition to the adversarial loss.
